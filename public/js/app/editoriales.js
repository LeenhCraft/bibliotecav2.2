...
$(document).ready(function () {
  tb = $("#tb").dataTable({
    ...
    ajax: {
      url: base_url + "admin/editoriales",
      method: "POST",
      dataSrc: "",
    },
    columns: [
...
function fntEdit(id) {
  let ajaxUrl = base_url + "admin/editoriales/search";
  ...
  $.post(ajaxUrl, { id: id }, function (data) {
    // console.log(data);
    if (data.status) {
      $("#id").val(data.data.ideditorial);
      $("#name").val(data.data.edi_nombre);
      $("#description").val(data.data.edi_descripcion);
      $("#status").val(data.data.edi_estado);
    } else {
      Swal.fire({
        title: "Error",
        text: data.message,
...
function fntDel(idp) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      let ajaxUrl = base_url + "admin/editoriales/delete";
...
function update(ths, e) {
  ...
  let ajaxUrl = base_url + "admin/editoriales/update";
...
function save(ths, e) {
  ...
  divLoading.css("display", "flex");
  let ajaxUrl = base_url + "admin/editoriales/save";
  $.post(ajaxUrl, form, function (data) {
    if (data.status) {
      // $("#modal").modal("hide");
      $("#id").val("");
